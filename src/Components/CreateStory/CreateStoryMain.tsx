...
interface CreateStoryMainProps {
    formSubmitted: boolean;
    setFormSubmitted:  (value:boolean) => void;
}

const CreateStoryMain : React.FC<CreateStoryMainProps> = ({formSubmitted, setFormSubmitted}) => {
    const isMobile = useMediaQuery("(max-width: 900px)");
    const isMedium = useMediaQuery("(min-width: 901px) and (max-width: 1501px)")
    const [isStorylineActive, setIsStorylineActive] = React.useState<boolean>(true)
  ...
  return (
    <Box sx={{
        ...
        <Box className="Option-to-proceed-btn" 
        sx={{
            display:"flex", 
            gap:"20px", 
            m:"40px 0px"
        ...
        >
            <Box className="storyline-option" 
            sx={{
                textAlign:"center",
                cursor:"pointer",
                backgroundColor: isStorylineActive ? "rgba(40, 40, 40, 0.29)" : "",
                p: isMobile ? "10px" : "30px 60px",
                borderRadius:"24px"
            }}
            onClick= {onStorylineClickHandler}
            >
                <Box component="img" src={storylineImage}
                    sx={{
            ...
            <Box className="main-character-option" 
            sx={{
                textAlign:"center",
                cursor:"pointer",
                backgroundColor: !isStorylineActive ? "rgba(40, 40, 40, 0.29)" : "",
            ...
            >
                <Box component="img" src={mainCharacterImage}
                    sx={{
                        width: isMobile ? "100px" : "auto"
                    }}
                />
